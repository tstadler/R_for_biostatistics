...
ggplot(salaries, aes(x=sl)) + 
  ...
  geom_density(alpha=.2, fill="#FF6666",adjust=.2) +   # overlay with transparent density plot filled in red where adjust                                                                  changes the kernel bandwidth, ?Q:Why is the colour here not given as                                                                           'red'
...
If you want to change the axis label format you can use the library scales
```{r}
library(scales) # load the library for axis formatting
fig + scale_y_continuous(labels=comma) # set the y-axis to non scientific notation
```
...
```{r}
ggplot(salaries, aes(x=sl, fill=sx)) +     # fill determines which variable the data is grouped by ?Q:diff between fill and                                                        colour? Later colour is uses
...
Create a histogram of the number of admissions into the hospital. Choose a suitable number of bins or binsize. Add a axis labels. Also add the mean number of admissions to the histogram with a green solid line.
...
Now, compute group means and standard deviations over the adimssions (easir to understand if you ad the measure variable here explicitly) for all combinations of moon phase and season. Plot a point plot with s.d. error bars which shows how hospital admissions vary as a function of season (x-axis) and moon phase. 
